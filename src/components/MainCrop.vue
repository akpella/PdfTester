    ...
    <div class="w-1/2">
      ...
      <div v-if="Object.keys(jsonData).length">
        ...
        <div v-for="data in jsonData.pages[page - 1].testCase" v-bind:key="data.id" class="mb-2">
          ...
          <label class="block text-gray-700 text-sm font-bold mb-2" for="expectedValue">Similarity Percentage:</label>
          <input
            class="shadow appearance-none border rounded w-1/2 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
            id="expectedValue" v-model="data.similarityPercentage">
        </div>
      </div>
    </div>
    <div class="w-1/2">
